...
public class RcuRoomStatusInitial extends AbstractAPIProcess {
	...
	protected Object process() throws SiDCException, Exception {
		...
		for (RcuRoomStatusRequest roomEntity : entity.getRooms()) {
			for (RcuDeviceCatalogueBean catalogueEntity : roomEntity.getCatalogues()) {
				...
				case "AIR-CONDITION":
					new RcuHvacInitial(roomEntity.getRoomno(), catalogueEntity.getDevices()).execute();
					break;
				case "BULB":
					new RcuBulbInitial(roomEntity.getRoomno(), catalogueEntity.getDevices()).execute();
					break;
				case "CARD":
					new RcuCardInitial(roomEntity.getRoomno(), catalogueEntity.getDevices()).execute();
					break;
				case "SERVICE":
	...
	private void roomTypeInitial(RcuRoomStatusRequest entity) {
		HashMap<String, RcuRoomInfoBean> typeMap = (HashMap<String, RcuRoomInfoBean>) DataCenter.getInstance()
				.get(CommonDataKey.RCU_ROOM_INFO);
		LogAction.getInstance().debug("roomTypeInitial: step 1/3 get DataCenter data success");

		if (typeMap == null) {
			typeMap = new HashMap<String, RcuRoomInfoBean>();
			LogAction.getInstance().debug("roomTypeInitial: step 1.1/3 DataCenter data is null");
		}

		typeMap.put(entity.getRoomno(), new RcuRoomInfoBean(entity.getRoomno(), entity.getRoomType()));
		LogAction.getInstance().debug("roomTypeInitial: step 2/3 put on HasMap success");

		DataCenter.getInstance().put(CommonDataKey.RCU_ROOM_INFO, typeMap);
