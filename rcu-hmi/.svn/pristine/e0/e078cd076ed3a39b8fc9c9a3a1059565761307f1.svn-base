	...
	public void execute() throws SiDCException {
		...
		while (!udp.getSocket().isClosed()) {
			...
			try {
				...
				// WebSocketClient websocketRoomControlClient =
				// (WebSocketClient) DataCenter.getInstance()
				// .get(CommonDataKey.WEBSOCKET_CLIENT_ROOM_CONTROL);
				//
				// if (websocketRoomControlClient == null ||
				// !websocketRoomControlClient.isOpen()) {
				// websocketRoomControlClient = websocketClientProcess(new
				// WebSocketClient(), "rcucommand",
