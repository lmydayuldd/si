...
public class SpareItemUpdateProcess extends AbstractAuthAPIProcess {

	private final SpareItemUpdateRequest entity;
	private final String STEP = "2";

	public SpareItemUpdateProcess(final SpareItemUpdateRequest entity) {
	...
	protected Object process() throws SiDCException, Exception {
		// TODO Auto-generated method stub
		// 先記錄要刪的資料
		final List<String> photoNameList = PhotoManager.getInstance().seleceNameById(String.valueOf(entity.getItemid()),
				PhotoType.SPAREITEM.toString());

		SpareManager.getInstance().updateItem(entity.getItemid(), entity.getCategoryid(), entity.getStatus(),
				entity.getSequence(), entity.getPrice(), entity.getQty(), entity.getList(), entity.getPhotolist(),
				entity.isPhotoupdate());
		LogAction.getInstance().debug("step 1/" + STEP + ":update success(SpareManager|updateItem).");
		...
		if (entity.isPhotoupdate() && !photoList.isEmpty()) {
			...
			try {
				...
				LogAction.getInstance().debug("upload photo success.");
			} catch (Exception e) {
				// 備份
			...
			try {
				HttpClientUtils.sendPostWithDeletePhoto(sitsUrl + PageList.DELETE_PHOTO, folderList,
						PhotoType.SPAREITEM.toString(), photoNameList);

				LogAction.getInstance().debug("delete photo success.");
			} catch (Exception e) {
	...
	protected void check() throws SiDCException, Exception {
		...
		SpareManager.getInstance().itemUpdateCheck(entity.getCategoryid(), entity.getItemid());
	}
}
