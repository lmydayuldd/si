...
public class ChatTokenInsertProcess extends AbstractAPIProcess {
	private final ChatTokenRequest entity;

	public ChatTokenInsertProcess(final ChatTokenRequest entity) {
	...
	protected Object process() throws SiDCException, Exception {
		...
		final String token = TokenManager.getInstance().insertChatMeetingToken(1, entity.getUserid(),
				new Timestamp(cal.getTimeInMillis()), entity.getInfo());
		LogAction.getInstance().debug("step 1/1: insert success(TokenManager|insertChatMeetingToken|" + token + ").");
	...
	protected void check() throws SiDCException, Exception {
		...
		if (StringUtils.isBlank(entity.getUserid())) {
			throw new SiDCException(APIStatus.ILLEGAL_ARGUMENT, "illegal of request(user id).");
		}
		...
		if (!AccountManager.getInstance().checkId(entity.getUserid())) {
			throw new SiDCException(APIStatus.ILLEGAL_ARGUMENT, "this user id does not exist.");
		}
		if (!TokenManager.getInstance().checkExpiredWithChat(entity.getUserid())) {
			throw new SiDCException(APIStatus.ILLEGAL_ARGUMENT, "token does not expire.");
		}
