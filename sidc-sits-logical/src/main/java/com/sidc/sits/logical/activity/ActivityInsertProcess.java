 ...
 *
 */
public class ActivityInsertProcess extends AbstractAuthAPIProcess {
	final DateFormat formatter = new SimpleDateFormat("yyyy/MM/dd HH:mm");
	private final ActivityInsertRequest entity;
	private final String STEP = "3";

	public ActivityInsertProcess(final ActivityInsertRequest entity) {
	...
	protected Object process() throws SiDCException, Exception {
		...
		final int id = ActivityManager.getInstance().insert(entity.getType(), entity.getRepeattype(),
				...
				new Gson().toJson(entity.getRepeatfrequent()));
		LogAction.getInstance().debug("step 2/" + STEP + " :insert success(ActivityManager|insert).");
	...
	protected void check() throws SiDCException, Exception {
		...
		if (StringUtils.isBlank(entity.getRegistrationstarttime())
			...
			throw new SiDCException(APIStatus.ILLEGAL_ARGUMENT, "illegal of request(registration start time).");
		}
		if (!StringUtils.isBlank(entity.getRegistrationstarttime())
				&& StringUtils.isBlank(entity.getRegistrationendtime())) {
			throw new SiDCException(APIStatus.ILLEGAL_ARGUMENT, "illegal of request(registration end time).");
		}
		if (!StringUtils.isBlank(entity.getRegistrationstarttime())
				&& !StringUtils.isBlank(entity.getRegistrationendtime())) {
			...
			entity.setRegistrationendtime(null);
			entity.setRegistrationstarttime(null);
		}
		...
		for (final ActivityLangBean activityLangEntity : entity.getList()) {
			...
			if (activityLangEntity.getLocation().length() > 20) {
				throw new SiDCException(APIStatus.ILLEGAL_ARGUMENT, "illegal of request(location length).");
			}
